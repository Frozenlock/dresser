        ...
        track-usage (fn [method]
                      (fn [tx & args]
                        (if *disable-tx-counter*
                          ...
                          (let [tx-count (db/temp-get-in tx [tx-count-field])]
                            ...
                            (let [new-count (swap! *tx-count inc)
                                  tx (db/temp-assoc tx tx-count-field new-count)]
                              (apply method tx args))))))

        wrap-transact (fn [transact-method]
                        (fn [dresser f opts]
                          (reset! *tx-count 0)
                          (let [dresser (db/temp-assoc dresser tx-count-field 0)]
  ...
  (testing "Upsert"
    (let [tx (impl-f)]
      (is (thrown-with-msg?
           clojure.lang.ExceptionInfo
           #"Missing document ID"
           (db/upsert! tx :drawer1 {:a 1 :b "String"}))))))
...
(defn test--immutable-temp-data
  ...
  (testing "Immutable temp data"
    (let [dresser (impl-f)
          initial-temp-data (db/temp-get dresser)]
      (is (= {:data 1234}
             (db/temp-get (db/with-temp-data dresser {:data 1234}))))
      ;; Use the initial dresser. Its temp-data should not have been
      ;; modified by the previous action.
      (is (= initial-temp-data (db/temp-get dresser))))))
